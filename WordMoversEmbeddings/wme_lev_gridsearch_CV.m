...
if docemb_scheme == 2
    gamma_list = [1e-3 1e-2 5e-2 0.5 1.0 1.5];
elseif docemb_scheme == 1
    gamma_list = -1;
end

sample_size_list = [100:40:1500];
%
...
%         trainFeaX_random = trainFeaX_random/sqrt(R); 
        ...
        [trainFeaX_random, samples, train_emd_time] = modifiedRecursiveNystrom(X,weight_X, sample_size,kFunc);
...
%         fprintf('Finish computing testFeaX \n');
        ...
        lambda_inverse = [10000:10000:100000];
...
%                 s1 = '-s 2 -e 0.0001 -n 8 -q -c '; % liblinear with omp
    ...
    disp(info);
    savefilename = [filename '_lev_rd' num2str(randdoc_scheme) ...
    ...
    save('validation_save_dir/twitter_CV', '-v7.3');
end
delete(gcp);

...
import utils
from anchornet import AnchorNet

# from scipy.sparse.linalg import eig
def is_pos_def(x, tol=1e-8):
    return np.all(np.linalg.eigvals(x) > -tol)

def simple_anchor(data, similarity_matrix, K, num_imp_samples, runs=1, mode="normal", normalize="rows", expand=False):
    """
    ...
    for r in range(runs):
        sample_indices = AnchorNet(data, 5, int(num_imp_samples/5))
        ...
        C = similarity_matrix[sample_indices].T

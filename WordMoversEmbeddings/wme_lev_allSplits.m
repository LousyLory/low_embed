...
clear,clc
nthreads = 12; % set as many as your cpu cores
...
randdoc_scheme = 2;     % if 1, leverage score distribution 1
                        % if 2, leverage score distribution 2
...
DMin = 1;    
R_list = [500];%[4 8 16 32 64 128 256 512];
...
for jjj = 1:length(filename_list)
    ...
    for jj = 1:length(dataSplit_list)
        ...
        for j = 1:length(R_list)
            ...
            [trainData, testData, telapsed_fea_gen] = wme_GenFea_leverage(Data,gamma,R,dataSplit,...

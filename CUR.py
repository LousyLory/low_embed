...
def simple_CUR(similarity_matrix, K, num_imp_samples, runs=1, mode="normal", normalize="rows", expand=False):
    """
    simple implementation of:
    1. CUR decomposition of similarity matrices
    ...
    for r in range(runs):
        sample_indices_1 = np.sort(random.sample(
            list_of_available_indices, num_imp_samples))
        sample_indices_2 = np.sort(random.sample(
            list_of_available_indices, num_imp_samples))
        A = similarity_matrix[sample_indices_1][:, sample_indices_2]
        ...
        C = similarity_matrix[sample_indices_1].T
        R = similarity_matrix[sample_indices_2].T

        if mode == "eye":
            inv_A = np.eye(len(A))  # np.linalg.inv(A)
            pass
        if mode == "normal" or "eigI":
            inv_A = np.linalg.pinv(A) #- 0.01 * np.eye(len(A)))
            pass
        if mode == "mueye":
